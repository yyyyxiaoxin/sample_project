...
// ADC零漂校准值
static float offset_ia = 0.0f;
static float offset_ib = 0.0f;
...
static void curr_sense_calibrate(void)
{
    ...
    offset_ia = adc_read_avg(MOT0_A_ADC_CHANNEL, ADC_SAMPLE_NUM);
    offset_ib = adc_read_avg(MOT0_B_ADC_CHANNEL, ADC_SAMPLE_NUM);
    ESP_LOGI(TAG, "电流校准完成: A=%.3fV, B=%.3fV", offset_ia, offset_ib);
}
...
void curr_sense_get_currents(curr_sense_t *cs)
{
    ...
    // 读取平均电压
    float volt_a = adc_read_avg(MOT0_A_ADC_CHANNEL, 5);
    float volt_b = adc_read_avg(MOT0_B_ADC_CHANNEL, 5);

    // 减去零漂 → 转换为电流
    cs->current_a = (volt_a - offset_ia) * VOLTS_TO_AMPS;
    cs->current_b = (volt_b - offset_ib) * VOLTS_TO_AMPS;
